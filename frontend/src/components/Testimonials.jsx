...
const Testimonials = () => {
  ...
  return (
    <section ref={sectionRef} id="testimonials" className="py-24 md:py-32 bg-[#FDFBF7] overflow-hidden px-6 relative">
      <div className="container mx-auto max-w-7xl">

        {/* Header Section */}
        <div className="text-center max-w-2xl mx-auto mb-16 md:mb-20">
          <div className="flex items-center justify-center gap-4 mb-4">
            <span className="w-8 h-px bg-[#b88a44]"></span>
            <span className="text-[#b88a44] text-[10px] font-bold uppercase tracking-[0.4em]">Kind Words</span>
            <span className="w-8 h-px bg-[#b88a44]"></span>
          </div>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-serif text-[#1a110a]">
            Voices of <span className="italic text-[#b88a44]">Satisfaction</span>
          </h2>
        </div>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 ${showAll ? 'pb-24' : ''}`}>
          {reviews.map((t, i) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className={`bg-white p-10 md:p-12 rounded-[3rem] relative shadow-sm hover:shadow-xl transition-all duration-300 border border-[#b88a44]/10 flex-col justify-between h-full group ${!showAll && i >= 6 ? 'hidden' : 'flex'}`}
            >
              <Quote className="absolute -top-5 right-10 text-[#b88a44]/20 transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-12" size={80} />

              <div className="relative z-10 grow">
                <div className="flex gap-1 mb-6 text-[#b88a44]">
                  {[...Array(5)].map((_, index) => <Star key={index} size={14} fill="currentColor" className="opacity-90" />)}
                </div>
                <p className="text-[#2b1c11]/80 text-base md:text-lg font-light leading-relaxed mb-8 italic">
                  "{t.text}"
                </p>
              </div>

              <div className="border-t border-[#b88a44]/20 pt-6 flex items-end justify-between">
                <div>
                  <h4 className="font-serif font-bold text-[#1a110a] text-lg mb-1">{t.name}</h4>
                  <p className="text-[10px] text-[#b88a44] uppercase tracking-widest">{t.role}</p>
                </div>
                <span className="text-[9px] border border-[#b88a44]/30 text-[#b88a44] px-3 py-1 rounded-full uppercase tracking-wider bg-white">
                  {t.niche}
                </span>
              </div>
            </motion.div>
        ...
        {!showAll && (
          <div className="mt-12 flex justify-center">
            <button
              ...
              className="flex items-center gap-2 px-8 py-3 md:px-10 md:py-4 bg-white border-2 border-[#b88a44] text-[#b88a44] rounded-full font-bold uppercase tracking-widest text-xs hover:bg-[#b88a44] hover:text-white transition-colors shadow-sm"
            >
              See All Reviews
              <ChevronDown size={16} />
            </button>
      ...
      {showAll && isInView && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-100 animate-in slide-in-from-bottom-5 fade-in duration-300">
          <button
            ...
            className="flex items-center gap-2 px-8 py-4 md:px-10 md:py-4 bg-[#b88a44] text-white rounded-full font-bold uppercase tracking-widest text-[10px] md:text-xs shadow-[0_10px_30px_rgba(184,138,68,0.4)] hover:bg-[#1a110a] active:scale-95 transition-all"
          >
            Hide Reviews
            <ChevronUp size={18} />
          </button>
